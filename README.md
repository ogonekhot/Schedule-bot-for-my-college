...
`settings/schedule.json` используется как начальный резервный кеш. Свежие данные записываются в `data/schedule.json`, дата и цвет эталонной недели — в `data/reference.json`, а аккаунты добавленных пользователями групп — в `data/accounts.json`. В Docker каталог `data` подключён к постоянному volume.
